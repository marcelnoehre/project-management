...
const trashBinBoard = '[data-cy="tash_bin_board"]';
...
Cypress.Commands.add('clickTaskCancel', () => {
    ...
    cy.get('.column:first .task:first').as('firstTask').click().wait(waitTime);
...
Cypress.Commands.add('taskAssignedChange', () => {
    ...
    // Select an option (replace 'John Doe' with the actual value you want to select)
